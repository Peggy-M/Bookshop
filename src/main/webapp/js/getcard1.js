...
var categoryLink=frontPath+"/reduce/getreducecard.do";
var h="380";
var w="400";

//点击优惠券反馈
function getCard(mid,pid,height,width){
	currMid=mid;
	currPid=pid;
	var evt=getEvent();
...
function getCard(mid,pid,height,width,isReadtype){
	currMid=mid;
	currPid=pid;
	var evt=getEvent();
	var element=evt.target||evt.srcElement;var objH=height==null||height==""?h:height;var objW=width==null||width==""?w:width;var layer=document.getElementById("tb_layer");if(isReadtype){categoryLink=frontPath+"/reduce/getreducecard.do";}else{categoryLink=frontPath+"/reduce/getreducecard.do"+"?&";}
...
var obj=document.createElement("div");obj.id="tb_layer";obj.style.cssText="position:absolute;z-index:200;";obj.style.height=height+"px";obj.style.width=width+"px";obj.style.top=height-5+"px";if((width-elWidth)/2>document.body.offsetWidth-elWidth-left){obj.style.left=left-(width-elWidth)+"px";}else if((width-elWidth)/2>left-document.body.offsetLeft){obj.style.left=left+"px";}else{obj.style.left=left-((width-elWidth)/2)+"px";}
