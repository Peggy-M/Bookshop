    ...
    <tbody>
      ...
      <div class="pagination" style="float: right;">
			<ul class="pagination right">
				...
				<c:if test="${pageno >1}">
				<li><a href="<%=basePath%>/address/addresslist.do?pageno=${pageno-1}">&laquo;</a></li>
				</c:if>
				...
				<c:forEach var="item" begin="1" end="${count }" step="1">
					<c:choose>
					    ...
					    <c:otherwise>
					    <li><a href="<%=basePath%>/address/addresslist.do?pageno=${item}">${item }</a></li>
					    </c:otherwise>
					</c:choose>
					    	
				</c:forEach>
				  <c:if test="${pageno < count}">
				  <li><a href="<%=basePath%>/address/addresslist.do?pageno=${pageno+1}">&raquo;</a></li>
				  </c:if>
...
function query(id){
	var id = $("#userId").val();
	window.location.href="<%=basePath%>/address/useraddress.do?id="+id;
}
