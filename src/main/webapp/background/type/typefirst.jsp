...
<table class="table table-bordered table-striped text-center bg-info">
                        ...
                        <div class="modal fade" id="add1" tabindex="-1" role="dialog" aria-labelledby="add11">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form id="addinfo" role="form" action="<%=basePath%>/type/add.do" method="post">
	...
	<thead>
      <tr class="info">
         ...
         <th class="text-center">一级类目名称</th>
         <th class="text-center">操作</th>
      </tr>
    </thead>
    <tbody>
      <c:forEach items="${firstTypeList}" var="firstType">
      <tr>
        <td>${firstType.typeId}</td>
        <td>${firstType.typeName}</td>
        <td>
           <a href="javascrip:void(0);" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#edit${firstType.typeId}" onclick="getFirstType(this,${firstType.typeId})">编辑</a>
           <!--修改-->
           <div class="modal fade" id="edit${firstType.typeId}" tabindex="-1" role="dialog" aria-labelledby="edit">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <ul>
                                            <li>
                                                <label><span>类目名称 </span></label>
                                                <input id="type_name${firstType.typeId}" name="typeName" type="text"/>
                                            </li>
                                        </ul>
                                    </div>
                                    <div class="modal-footer">
                                        ...
                                        <button onclick="updateFirstType(this,${firstType.typeId})" class="btn btn-primary btn-sm">保存</button>
                                </div>
                            </div>
                        </div>
           <shiro:hasRole name="超级管理员">
           <a class="btn btn-primary btn-sm" onclick="deleteItem(this,${firstType.typeId})">删除</a>
           </shiro:hasRole>
        </td>
      </tr>
      </c:forEach>
      ...
      <div class="pagination" style="float: right;">
			<ul class="pagination right">
				...
				<c:if test="${pageno >1}">
				<li><a href="<%=basePath%>/type/typelist.do?pageno=${pageno-1}">&laquo;</a></li>
				</c:if>
				...
				<c:forEach var="item" begin="1" end="${count }" step="1">
					<c:choose>
					    ...
					    <c:otherwise>
					    <li><a href="<%=basePath%>/type/typelist.do?pageno=${item}">${item }</a></li>
					    </c:otherwise>
					</c:choose>
					    	
				</c:forEach>
				  <c:if test="${pageno < count}">
				  <li><a href="<%=basePath%>/type/typelist.do?pageno=${pageno+1}">&raquo;</a></li>
				  </c:if>
...
function deleteItem(item,typeId){
	...
	if (bool){
		var aTag = item;
		aTag.href = "<%=basePath%>/type/delete.do?typeId="+typeId;
		}
	}
	
//获取当前编辑内容
function getFirstType(item,typeId){
	$.post(
		"<%=basePath%>/type/getTypeInfo.do?id="+typeId,
				{
