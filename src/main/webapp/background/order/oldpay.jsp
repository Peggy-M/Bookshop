...
<c:if test="${null==orderList}">
		<script type="text/javascript">
			window.location.href = "<%=basePath %>/order/oldOrder.do";
		</script>
	</c:if> 
    ...
    <tbody>
      ...
      <tr>
       ...
       <%--  <shiro:hasRole name="超级管理员">
         ...
         <a href="<%=basePath%>/order/getLogistis.do?orderId=${order.orderId}&id=${order.orderBak}&company=${order.orderDeliver}" class=" btn btn-primary">查看物流详情</a>
        </td>
      </tr>
      </c:forEach>
      ...
      <div class="pagination" style="float: right;">
			<ul class="pagination right">
				...
				<c:if test="${pageno >1}">
				<li><a href="<%=basePath%>/order/oldOrder.do?pageno=${pageno-1}">&laquo;</a></li>
				</c:if>
				...
				<c:forEach var="item" begin="1" end="${count }" step="1">
					<c:choose>
					    ...
					    <c:otherwise>
					    <li><a href="<%=basePath%>/order/oldOrder.do?pageno=${item}">${item }</a></li>
					    </c:otherwise>
					</c:choose>
					    	
				</c:forEach>
				  <c:if test="${pageno < count}">
				  <li><a href="<%=basePath%>/order/oldOrder.do?pageno=${pageno+1}">&raquo;</a></li>
				  </c:if>
...
<%-- 
//保存订单号
	function updateBak(item,com,id){
