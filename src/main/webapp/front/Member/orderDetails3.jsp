...
<div class="orderDet">
	<div class="orderDet1">
		...
		<div class="box2">
			...
			<div class="each"><img src="<%=basePath %>/img/memOrderDet_img_step3.jpg" alt=""/><br/>02.等待发货</div>
			<div class="ic"><img src="<%=basePath %>/img/memOrderDet_img_arr.gif" alt=""/></div>
			<div class="each"><img src="<%=basePath %>/img/memOrderDet_img_step2.jpg" alt=""/><br/>03.商品出库</div>
			<div class="ic"><img src="<%=basePath %>/img/memOrderDet_img_arr.gif" alt=""/></div>
...
<div class="orderDet2">
	...
	<div class="orderDet_infoBox">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" id="table8">
			<col width="455"/>
			<tr>
			<td>发货信息：<a id="showDeliver" style="cursor: pointer;">点击查看物流信息</a>
			</td>
			</tr>
			<tr>
			</tr>
...
<div class="orderTotal">
	...
	<div class="total" style="float: left;display: inline;display: inline;margin-left: 80px;width: 70px;height: 30px;text-align: center;border-radius: 8px;background: #FF5080;">
		<a style="color: #fff;line-height: 2;cursor: pointer;" onclick="confirmAcceptProduct(${order.orderId})">确认收货</a>
	</div>
	<div class="total" style="float: right;margin-right: 50px;">订单支付金额：<span>￥${order.orderPrice/100 }</span></div>
</div>
...
<input type="hidden" id="orderBak" value="${order.orderBak}">
<input type="hidden" id="deliver" value="${deliver}">

<script type="text/javascript">
	/* start-- 查看物流信息 */
	//点击新增按钮
	$('#showDeliver').click(function() {
		var orderId = $('#orderId').val();	//订单编号
		var orderBak = $('#orderBak').val();	//快递单号
		var deliver = $('#deliver').val();	//快递单号
		var url = '<%=basePath%>/Member/getLogistis.do?orderId='+orderId+'&id='+orderBak+'&company='+deliver;
		$('#dialogContent').slideDown();
		$('#dialog').slideDown();
		$('#addAddrFrame').attr('src',url);
	});
	...
	//点击确认收货
	function confirmAcceptProduct(orderId) {
		var result = confirm('是否确认收货?');
		if (result) {
			window.location.href = '<%=basePath%>/Member/confirmAccept.do?orderId='+orderId;
		}
	}
</script>
