...
<div class="memRight">
    ...
    <div class="mem_myOrderTab">
		    <a href="javascript:void(0);" class="current">全部订单</a>
		    <a href="javascript:void(0);" >未支付订单</a>
            <a href="javascript:void(0);" >未确认订单</a>
           <a href="javascript:void(0);" >历史订单</a>
	 </div>
        
     <div class="myorder_sea" style=>
        <form action="javascript:void(0);" method="post" style="margin:0px;">
          <input type="hidden" name="ColumnId" value="-1" />
	      <div class="number">订单号：
	      <input type="text" name="aliasCodeInput" id="aliasCodeInput" value="" class="put" style="width:225px;"/>
	      </div>
	      
		  <!--  <div class="time">
		         下单时间：  
		   <input type="text" id="beginDateInput" readonly="readonly" name="beginDateInput" value="" class="put"/>
		   <img id="beginDateBtn" style="cursor:pointer;" src="/style/guanggou/images/huihis_timeimg.gif" alt="" />
		        到
		   <input type="text" id="endDateInput"  readonly="readonly" name="endDateInput" value="" class="put"/>
		   <img id="endDateBtn" style="cursor:pointer;" src="/style/guanggou/images/huihis_timeimg.gif" alt="" />
		   </div> -->

           <div class="clear"></div>
            <div tabindex="-1" class="selDiv">
                <label class="x-form-item-label">订单状态：</label>
                <select id="type1" name="state">
                    <option value="-1">处理状态</option>
                    <option value="0">未处理</option>
                    <option value="1">已取消</option>
                    <option value="2">用户已确认</option>
                    <option value="3">缺货中</option>
                    <option value="4">已经送货</option>
                    <option value="6">已完成</option>
                </select>
            
                <select id="type2" name="state">
                    <option value="-1">订单支付状态</option>
		   ...
		   <div class="btn" style="padding:6px 0 0 20px;">
		   		<input type="image" src="<%=basePath %>/img/mem_orderSea.gif" alt="" />
		   </div>
		   <div class="clear"></div>
        </form>
	 </div>
	  
	 <div class="hisOrder_box">
     
		<table class="orderListTab" width="100%" cellpadding="0" cellpadding="0">
			  <tr class="title">
			    <td>订单号</td>
			    <td>下单时间</td>
			    <td>订单金额</td>
			  ...
			  <c:forEach items="${orderList }" var="order">
	      		  <tr>
	        		<td class="td1">${order.orderNumber }</td>
			        <td>${order.orderCreateTime }</td>
			        <td>￥${order.orderPrice/100 }</td>
	        		<td class="td4">${order.orderStatus }</td>
	        		<td>${order.orderDeliver }</td>
	        		<td><a href="<%=basePath%>/Member/orderDetails.do?orderId=${order.orderId}"><img src="<%=basePath %>/img/mem_orderDetail.gif" alt=""/></a></td>
			        <!-- <td></td> -->
	      		  </tr>
      		  </c:forEach>
		</table>
