   ...
   <div class="myadd_m1">
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <c:forEach items="${addressList }" var="address">
            <tr class="bg" >
              <td class="td2">${address.addressUserName }</td>
              <td class="td3">
			     <div>手机：${address.addressPhone } <span>固话：</span></div>
				 <div>地址：${address.addressArea }&nbsp;&nbsp;${address.addressDetails }</div>
				 <div>邮编：${address.addressPostcode }</div>
			  </td>
              <td class="td4">
              <span><a href="javascript:void(0);" class="thickbox" onclick="updateAddress(${address.addressId })">[修改]</a></span>
               &nbsp;<a href="javascript:;" onclick="deleteAddress(${address.addressId })">[删除]</a></td>
           </tr>
        </c:forEach>
     </table>
    </div>
    
	 <div class="myadd_title"><a id="addAddress" style="cursor: pointer;">新增收货地址</a><span>温馨提示：在填写订单信息界面，可以直接选择地址薄中的地址</span></div>
    ...
    <script type="text/javascript">
    	...
    	function updateAddress(addressId) {
    		$('#dialogContent').slideDown();
    		$('#dialog').slideDown();
    		$('#addAddrFrame').attr('src','<%=basePath %>/Member/update.do?addressId='+addressId);
		}
    	...
    	function deleteAddress(addressId) {
			var result = confirm('确定删除此收货地址?');
			if(result){
				var url = '<%=basePath%>/Member/deleteAddress.do';
				$.post(
					url,
					{
						addressId:addressId
