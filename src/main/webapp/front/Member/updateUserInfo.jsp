...
<div class="memRight">
     <h1 class="memRightTitle"><div>我的资料</div></h1>
      <form name="userForm" id="userForm" action="" method="post" style="margin:0px;" enctype="multipart/form-data" onSubmit="return false;">
	  	<!-- <div class="myinfo_m1" style="display:none">
        	<div class="img">
              <img id="fileChecker" name="fileChecker" src="/style/guanggou/case/hui_100_100.jpg" alt=""/>
        	</div>
		<div class="right">
		  <div class="t1">选择你的个性头像，<br />请点击“浏览”后，选择需要上传的图片！</div>
		  <input type="hidden" name="providerInnerName" value="userBaseDatum"/>
          <input type="hidden" name="deleteTopPic" id="deleteTopPic"/>
          <input type="hidden" name="Email" value="309673435@qq.com"/>
          <input type="hidden" name="mobilePhone" value="15872424280"/>
          <input type="hidden" name="Gender" value="-1"/>
          <div class="t2">上传新头像：<input id="picfile" type="file" name="TopPicture_file" onchange="changeSrc(this)"/></div>
          <div class="t3">要求：图片尺寸最大100*100像素，文件大小100k以内（支持JPG，GIF）。</div>
		</div>
	  </div> -->

      <div class="myinfo_title">我的基本资料：</div>
	  <div class="myinfo_m2">
	    <div class="myinfo_m2l">用 户 名：</div>
	    <div class="myinfo_m2r5">
	    	<a class="__cf_email__" href="javascript:void(0);">${userInfo.userName }</a>
	    </div>
	    <div class="myinfo_m2l">E-mail：</div>
	    <div class="myinfo_m2r">
	    	<input type="text" id="Email" name="Email" value="${userInfo.userEmail }"/>&nbsp;&nbsp;
	    	<a href=""></a>
	    </div>
	    <div class="myinfo_m2l">真实姓名：</div>
	    <div class="myinfo_m2r2"><input type="text" id="realName" name="realName" value="${userInfo.userRealname }" disabled="disabled"/></div>
	    <div class="myinfo_m2r3">( 为保障您的利益，请务必填写真实姓名，因为领取奖金、奖品和收货都要验证  真实身份。真实姓名设置以后不可修改，需要修改请联系客服。)</div>
	    <div class="myinfo_m2l">手&nbsp;&nbsp;&nbsp;&nbsp;机：</div>
	    <div class="myinfo_m2r">
	    	<input type="text" id="mobilePhone" name="mobilePhone" value="${userInfo.userPhone }"/>&nbsp;&nbsp;<a href=""></a></div>
		<div class="clear"></div>
      </div>
      
      <div class="myinfo_btn"><input type="image" src="<%=basePath %>/img/myinfo_btnOK.gif" onclick="updateUserInfo()"/></div>
     </form>
     <div id="showMsg"></div>
     </div>
    <div class="clear"></div>
...
<script type="text/javascript">
	...
	function updateUserInfo() {
		...
		if (email==""||email==null) {
			alert('请输入邮箱');
		}else if (mobilePhone==""||mobilePhone==null) {
			alert('请输入手机号');
		}else {
			$.post(
				'<%=basePath%>/Member/updateUserInfo.do',
				{
					userEmail:email,
				...
				function(data) {
					if (data=='success') {
						alert('修改成功');
						window.location.href = '<%=basePath%>/Member/index.do';
					}else {
						alert('修改失败');
					}
				}
